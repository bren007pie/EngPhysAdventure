...
from GameFunctions import *  # importing the global dictionaries/values, do we need this? or can we just import gameclasses?
import time
...
def sidequests(MAPS, PLAYER, ITEMS, INTERACT, QUESTS, ENEMIES, GAMEINFO, GAMESETTINGS):
    ...
    if INTERACT['coat of arms'].quest and QUESTS["secret spaces"]:  # Unlocks the secret space once you get the scroll
        MAPS[0][2][1][0].remove_wall("d")  # DON'T FORGET to make wall a list instead of a tuple in the object!
    ...
    if INTERACT["tri-coloured glasses"].quest and QUESTS['national treasure']:  # Once the sign is read
        MAPS[1][0][1][0].remove_wall("u")  # DON'T FORGET to make wall a list instead of a tuple in the object!
    ...
    if INTERACT["lit firepit"].quest and QUESTS['open the cabin']:
        MAPS[8][9][0][4].remove_wall("r")  # DON'T FORGET to make wall a list instead of a tuple in the object!
        QUESTS['open the cabin'] = 0

    if INTERACT['gate of the forest'].quest and QUESTS['open the forest']:
        MAPS[0][7][0][4].remove_wall("b")  # DON'T FORGET to make wall a list instead of a tuple in the object!
...
def story(MAPS, PLAYER, ITEMS, INTERACT, QUESTS, ENEMIES, GAMEINFO, GAMESETTINGS):
    ...
    if ENEMIES["devan the most unhelpful"].spoke and QUESTS["talk to devan"]:
        MAPS[1][3][1][0].remove_enemy(ENEMIES["devan the most unhelpful"])  # removes him from infront of hatch
    ...
    if ENEMIES['dr. minnick'].quest and QUESTS["minnick get oscilloscope"]:
        MAPS[6][1][1][0].remove_wall("d")  # DON'T FORGET to make wall a list instead of a tuple in the object!
...
def events(MAPS, PLAYER, ITEMS, INTERACT, QUESTS, ENEMIES, GAMEINFO, GAMESETTINGS):
    ...
    if GAMEINFO['winner'] and QUESTS['completionist']:
        ...
        if (False in enemycompletion) or (False in interactcompletion):
            ...
            printT("(\S) YOU DID IT!!!! YOU " + wincolour + "100% THE GAME" + textcolour + "! Type anything to continue:")
            input("")
    ...
    if not ENEMIES['brendan fallon'].alive and QUESTS['kill brendan fallon']:
        printT("(\S) YOU DID IT!!!! YOU " + wincolour + "KILLED BRENDAN FALLON" + textcolour + "! Type anything to continue:")
        input("")
        AsciiArt.Acheivement()
        GAMEINFO['log'].append("---THEY KILLED BRENDAN FALLON!---")  # adds to log file for the clout
        save_game(GAMEINFO['playername'] + " killed bf")  # save to be submitted
        gmfourtwenty = 1562765901.005 + 24240 - 141 - (4 * 60 * 60)  # 4:20pm, Subtracting the 4 hours for gm time
        insttime = time.gmtime(gmfourtwenty)  # Sets time to constantly 4:20pm time object, opens PAP
    ...
    # Event Main Activity "Pack-a-Punching" when you inspect the machine
        ...
        while PAPScreen:
            ...
            # Displaying Options
            if upgradechoice == 0:
                printT("Item 1: Choose an " +itemcolour+ "Item" +textcolour+ " to " +wincolour+ "Upgrade" +textcolour+ "")
            else:
                printT("Item 2: Choose an " +textcolour+ "Item" +textcolour+ " to " +losecolour+ "Sacrifice" +textcolour+ "")
            k = 0
            for i in PLAYER.inv:
                ...
                if (k != int(upgradechoice)):
                    printT("[" + str(k) + "]" + PLAYER.inv[i].name + " " + str(PLAYER.inv[i].stats))
            printT("[" + str(k + 1) + "]Back\n")
            # Input and Check Input
            try:
                if upgradechoice == 0:
                    printT("Choose the number of the " +itemcolour+ "item" +textcolour+ " you want to " +interactcolour+ "Pack-a-Punch" +textcolour+ ": ")
                    upgradechoice = eval(input(""))
                    if upgradechoice <= 0 or upgradechoice > k + 1:
                        printT("" +losecolour+ "Please enter a valid option!" +textcolour+ "")
                        upgradechoice = 0
                else:
                    printT("Choose the number of the " +itemcolour+ "item" +textcolour+ " you want to " +losecolour+ "sacrifice" +textcolour+ ": ")
                ...
                printT("" +losecolour+ "Please input a number selection!" +textcolour+ "")
            ...
            if (0 < upgradechoice < k + 1) and (0 < sacrificechoice < k + 1):
                ...
                k = 0
                for i in PLAYER.inv:
                    if PLAYER.inv[i].name == 'EMPTY':  # skips empty items by naming. Making assumption that no item_object is named 'EMPTY'
                        # print "THIS B**** EMPTY - YEET"
                        continue  # advance to the next i
                    k += 1
                    if k == upgradechoice:
...
def move_script(place, MAPS, PLAYER, ITEMS, INTERACT, QUESTS, ENEMIES, GAMEINFO, GAMESETTINGS):
    ...
    if bf.location != (None, None, None, None):
        MAPS[bf.location[0]][bf.location[1]][bf.location[2]][bf.location[3]].remove_enemy(bf)
    if random() <= bfchance:
...
def attack_script(enemy, MAPS, PLAYER, ITEMS, INTERACT, QUESTS, ENEMIES, GAMEINFO, GAMESETTINGS):
    ...
    if type(enemy) is Animal:  # if you try to attack an animal BH comes and punches you
        printT(" (\S)An oblivion gate opens and a " + lightmagenta + "purple faced hero" + textcolour + " in " + lightblack + "ebony armour" + losecolour + "punches you" + textcolour + " for attempting to attack an animal.")
        PLAYER.health -= 25
